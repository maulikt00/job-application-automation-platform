...
def test_application_answer_many_to_many(session_factory) -> None:
    ...
    with session_scope(session_factory) as session:
        app = session.get(ApplicationORM, app_id)
        answer = session.get(AnswerORM, answer_id)
        app.answers.append(answer)

    with session_scope(session_factory) as session:
        loaded = session.get(ApplicationORM, app_id)
        assert [a.question_key for a in loaded.answers] == ["why-us"]
